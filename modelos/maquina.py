...
def criar_maquina(maquina: Maquina) -> int:
    ...
    try:
        ...
        new_id = cur.lastrowid  #pega o id gerado pelo autoincrment
...
def atualizar_maquina(id_maquina: int, campos: dict) -> bool:   #campos: dicionario com colunas e dados a atualizar
    ...
    try:
        ...
        conn.commit()
        affected = cur.rowcount #indica quantas linhas alteradas, 
        cur.close()
        return affected > 0  #retorna true se alterou algo, false se não 
    finally:
        conn.close() #fecha a conexão
...
def deletar_maquina(id_maquina: int) -> bool:
    ...
    try:
        ...
        cur.close()
        return affected > 0  #retorna true se a maquina foi deletada
    finally:
        conn.close() #fecha a conexão
...
def listar_maquinas_por_lavanderia(id_lavanderia: int) -> List[Maquina]:
    ...
    try:
        ...
        for row in cur.fetchall():    #para cada linha cria um objeto maquina e adiciona ele na lista
